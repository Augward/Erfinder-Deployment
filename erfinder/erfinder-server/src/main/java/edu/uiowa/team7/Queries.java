    ...
    public static Connection GetConnection() {
        if (SQLConnection != null) {
            try {
                ...
                logger.warn("Database connection check failed. Reconnecting...", e);
            }
        }
        try {
            ...
            logger.error("Fatal connection error", e);
        }
        return null;
    }
    ...
    public static boolean ApproveUser(String targetUserID) throws SQLException {
        try (Connection c = GetConnection()) {
            try (PreparedStatement insertStmt = c.prepareStatement(
                ...
                if (inserted > 0) {
                    try (PreparedStatement deleteStmt = c.prepareStatement("DELETE FROM registration WHERE userid = ?")) {
